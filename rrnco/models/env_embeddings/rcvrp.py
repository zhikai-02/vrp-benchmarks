...
class DistanceExpert(nn.Module):
    ...
    def _sample_indices(self, distance, phase, B, N):
        if self.sample_type == "random":
            ...
            # If N < sample_size, we must sample with replacement
            replacement = self.sample_size > probabilities.size(-1)
            sampled_indices = torch.multinomial(
                probabilities, self.sample_size, replacement=replacement
            )  # [B, N, sample_size]
