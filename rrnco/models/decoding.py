...
def unbatchify_and_gather(x, indices, num_starts):
    x_unbatch = unbatchify(x, num_starts)
    batch_size = x_unbatch.shape[0]
    batch_indices = torch.arange(batch_size, device=indices.device)
    return x_unbatch[batch_indices, indices]
...
class DecodingStrategy(metaclass=abc.ABCMeta):
    ...
    def step(
        ...
        **kwargs,
        ...
        # Fix for batch_size=1 resulting in scalar logprobs
        if logprobs.dim() == 0:
